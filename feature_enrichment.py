...
def basic_enrichment(train, test, helper_data_path):
    ...
    for df in [train, test]:
        ...
        item_seq_number_count = df['item_seq_number'].value_counts()
        df['item_seq_number_count'] = df['item_seq_number'].apply(lambda isn: item_seq_number_count[isn])
                
        param_1_count = df['param_1'].value_counts()
        df['param_1_count'] = df['param_1'].apply(lambda param: param_1_count[param])

        param_2_count = df['param_2'].value_counts()
        df['param_2_count'] = df['param_2'].apply(lambda param: param_2_count[param])

        param_3_count = df['param_3'].value_counts()
        df['param_3_count'] = df['param_3'].apply(lambda param: param_3_count[param])
